...
(function() {
    ...
    window.fetch = async function(...args) {
        ...
        if (isProtected) {
            if (!currentUser) {
                ...
                // 失敗したPromiseを返すことで、呼び出し元のcatchブロックでエラーをハンドリングできるようにする
                return Promise.reject(new Response(JSON.stringify({ error: 'Authentication required.' }),{ status: 401, statusText: 'Unauthorized' }));
            }
            const token = await getIdToken();
            if (!token) {
                 console.error('保護されたAPIへのリクエストがブロックされました: トークン取得失敗');
                return Promise.reject(new Response(JSON.stringify({ error: 'Failed to retrieve auth token.' }), { status: 401, statusText: 'Unauthorized' }));
            }
    ...
    function updateUIForAuthState(user) {
        ...
        if (user) {
            ...
            // ログイン時にギャラリーのタブがアクティブでなければ、更新を試みる
            if (typeof loadR2Gallery === 'function') {
                // ログイン直後に一度だけ更新をかける
                if (!r2GalleryTab.classList.contains('active')) {
                   setTimeout(() => loadR2Gallery(), 500);
                }
            }
...
let currentCombinedAudioInfo = { text: '', filename: '' };
let currentPreviewAudioBlob = null;
let currentPreviewAudioInfo = { text: '', filename: '' };
...
function renderR2Gallery() {
    ...
    filteredFiles.forEach(file => {
        ...
        const formatConfig = getFormatConfigByContentType(file.contentType);
        const downloadFilename = createSafeFileName(file.modelId, file.text, formatConfig.extension);
        ...
        card.innerHTML = `
            ...
            <div class="player-actions">
                 <a href="/api/get/${encodeURIComponent(file.key)}" download="${downloadFilename}" class="icon-btn download-link" title="ダウンロード">📥</a>
...
async function loadR2Gallery() {
    ...
    try {
        // fetchWithAuthは不要。auth.jsが自動でfetchを処理する
        ...
        const filesFromD1 = await response.json();
        
        allR2Files = filesFromD1.map(file => {
            const extension = file.r2_key.split('.').pop() || 'bin';
            return {
                key: file.r2_key,
                modelId: file.model_name,
        ...
        const currentSearchValue = r2SearchModelSelect.value;
        ...
        r2SearchModelSelect.value = currentSearchValue;
...
const setupEventListeners = () => {
    ...
    generateBtn.addEventListener('click', async () => {
        ...
        if (successfulBlobs.length > 0) {
            ...
            if(combinedBlob) {
                ...
                currentCombinedAudioInfo.filename = createSafeFileName(getCurrentModelId(), combinedText, formatConfig.extension); // for download
                
                downloadCombinedBtn.onclick = () => {
                    const a = document.createElement('a'); a.href = url;
                    a.download = currentCombinedAudioInfo.filename;
    ...
    generatePreviewBtn.addEventListener('click', async () => {
         ...
         currentPreviewAudioInfo.filename = createSafeFileName(getCurrentModelId(), combinedTextForFilename, formatConfig.extension); // for download
    ...
    async function saveAudioToR2(blob, modelId, text, button) {
        if (!blob || !modelId || !text) {
             setStatus('保存対象の音声データが不完全です。', true);
             return;
        }
        setStatus('R2に音声を保存中...');
            ...
            reader.onloadend = async () => {
                try {
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(`R2への保存に失敗: ${errorData.error}`);
                    }
                    const result = await response.json();
                    setStatus(`音声をR2に保存しました (Key: ${result.key})`);
    ...
    r2GalleryContainer.addEventListener('click', async (e) => {
        ...
        const filename = downloadLink ? downloadLink.download : fileKey;

        if (confirm(`ファイル「${filename}」を完全に削除しますか？この操作は元に戻せません。`)) {
            try {
